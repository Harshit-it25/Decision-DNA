      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col justify-between">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 flex justify-between">
            ...
            <span className={metrics.psi < 0.1 && (metrics.featurePsi?.income || 0) < 0.1 && (metrics.featurePsi?.creditScore || 0) < 0.1 ? 'text-emerald-500' : 'text-rose-500'}>
              {metrics.psi < 0.1 && (metrics.featurePsi?.income || 0) < 0.1 && (metrics.featurePsi?.creditScore || 0) < 0.1 ? 'Stable' : 'Drift Alert'}
            </span>
          </h3>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <div className="text-2xl font-black text-white">{metrics.psi.toFixed(3)}</div>
              <p className="text-[10px] text-slate-500 uppercase">Output</p>
            </div>
            <div>
              <div className="text-2xl font-black text-white truncate">{(metrics.featurePsi?.income || 0).toFixed(3)}</div>
              <p className="text-[10px] text-slate-500 uppercase">Income</p>
            </div>
            <div>
              <div className="text-2xl font-black text-white truncate">{(metrics.featurePsi?.creditScore || 0).toFixed(3)}</div>
              <p className="text-[10px] text-slate-500 uppercase">Credit</p>
            </div>
          </div>
        </div>
