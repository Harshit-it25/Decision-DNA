...
async def get_security_insight_proxy(
    ...
    if not gemini_key or gemini_key == "admin" or gemini_key == "":
        return {"insight": "AI Insights unavailable: API key not configured on backend."}
    ...
    try:
        ...
        with urllib.request.urlopen(request_obj, timeout=10) as response:
            ...
            return {"insight": "No insight generated by AI model."}
            
    except HTTPError as e:
        error_content = e.read().decode("utf-8")
        logging.error(f"Gemini API returned HTTP error: {e.code} - {error_content}")
        if e.code == 429 or "RESOURCE_EXHAUSTED" in error_content:
            return {"insight": "⚠️ AI quota limit reached for today. Governance insights will resume automatically tomorrow, or upgrade your Gemini API plan at ai.google.dev."}
        return {"insight": f"AI insight unavailable: Gemini API returned status code {e.code}."}
    except Exception as e:
        logging.error(f"Gemini Proxy failed: {e}")
        return {"insight": f"AI insight unavailable: {str(e)}"}
