...
import { trainModel as apiTrainModel, runTest as apiRunTest } from './api/modelApi';
...
const App: React.FC = () => {
  ...
  const handleDriftCheck = async () => {
    ...
    if (allApplicants.length < 10) return;
    ...
    const newMetrics = detectDrift(current, baseline);
    
    // Inject some artificial drift for demonstration occasionally
    if (Math.random() > 0.8) {
      newMetrics.psi += 0.2;
    }
    ...
    if (newMetrics.psi >= 0.1) {
      const severity = newMetrics.psi >= 0.25 ? 'CRITICAL' : 'WARNING';
      const message = `Data drift detected! PSI is ${newMetrics.psi.toFixed(3)}. System stability may be compromised.`;
  ...
  const handleAttack = async (type: any, customParams?: any) => {
    ...
    if (newMetrics.psi >= 0.1) {
      addNotification(`Security Alert: ${audit.details}`, newMetrics.psi >= 0.25 ? 'error' : 'warning');
    }
