...
const Dashboard: React.FC<DashboardProps> = ({ activeModel, metrics, security, auditLogs, insight, aiTier }) => {
  ...
  const rfAccuracy = realMetrics?.random_forest_accuracy ?? activeModel.metrics.accuracy ?? 0;
  const lrAccuracy = realMetrics?.logistic_regression_accuracy ?? 0.825;
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          subValue={`v${activeModel.version}`}
        />
        <StatCard 
